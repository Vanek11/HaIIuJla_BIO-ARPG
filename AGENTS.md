...
2. **Ветка на каждую задачу.** Изменения и новые фичи делать в отдельной ветке
   (`feature/<название>`) от актуального `main`. В `main` сливать только после
   полного завершения и подтверждения пользователем (merge + push), т.к. Vercel
   деплоит из `main` — тестовые версии туда не должны попадать.

3. **Проверка перед коммитом.** Минимум: `node --check` для изменённых js-файлов;
